...
import static by.mishota.graduation.resource.FilePath.PATH_PAGE_ADMIN_DELETE_ALL_NEWS;
import static java.lang.Math.ceil;

public class DeleteAllNewsGetCommand implements ActionCommand {
    private static final Logger logger = LogManager.getLogger();
    private static final int numberPageDefault = 1;
    private static final int numberRowsOnPage = 5;

    private NewsService newsService;

    public DeleteAllNewsGetCommand(NewsService newsService) {
        this.newsService = newsService;
    }

    @Override
    public Router execute(HttpServletRequest request) {
        Router router = new Router(PagePathManager.getProperty(PATH_PAGE_ADMIN_DELETE_ALL_NEWS));
