...
import static by.mishota.graduation.resource.FilePath.PATH_PAGE_DELETE_PHOTOS;
import static by.mishota.graduation.resource.FilePath.PATH_PAGE_VIEW_PHOTOS;

public class DeletePhotosGetCommand implements ActionCommand {
    private static final Logger logger = LogManager.getLogger();

    private UserService userService;

    public DeletePhotosGetCommand(UserService userService) {
        this.userService = userService;
    }

    @Override
    public Router execute(HttpServletRequest request) {
        Router router = new Router(PagePathManager.getProperty(PATH_PAGE_DELETE_PHOTOS));
