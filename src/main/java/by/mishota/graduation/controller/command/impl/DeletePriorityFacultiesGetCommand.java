...
import static by.mishota.graduation.resource.FilePath.PATH_PAGE_DELETE_PRIORITY_FACULTIES;

public class DeletePriorityFacultiesGetCommand implements ActionCommand {
    private static final Logger logger = LogManager.getLogger();

    private FacultyService facultyService;

    public DeletePriorityFacultiesGetCommand(FacultyService facultyService) {
        this.facultyService = facultyService;
    }

    @Override
    public Router execute(HttpServletRequest request) {
        Router router = new Router(PagePathManager.getProperty(PATH_PAGE_DELETE_PRIORITY_FACULTIES));
